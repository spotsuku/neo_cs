// 派遣者・セッション・出席記録の **mock seed**。
//
// 型 (Participant / Session / AttendanceRecord / ParticipantFieldDef 等) と
// ロール解決ユーティリティ (resolveParticipantRole / inferRoleLevel) は
// `@/lib/master/participants` に移動済。新規コードはそちらから import すること。
...
// 後方互換 re-export: 旧来 `from "@/lib/mock/participants"` で type/utility を import
// している箇所のため。新規コードは `@/lib/master/participants` を使うこと。
export {
  type Participant,
  type Session,
  type AttendanceRecord,
  type ParticipantSeniority,
  type ParticipantFieldType,
  type ParticipantFieldDef,
  inferRoleLevel,
