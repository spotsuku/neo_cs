...
export type SurveysClientProps = {
  schedules: SurveySchedule[];
  surveys: Survey[];
  npsBySurvey: Record<string, number | undefined>;
...
export default function SurveysClient({
  ...
  }, [productFilter, respondentFilter, periodFilter, schedules, surveys, todayISO]);
        ...
        <section className="space-y-4">
          {groups.map((g) => (
            <ScheduleGroup
              ...
              schedule={g.schedule}
              items={g.items}
              respondentType={g.respondentType}
...
function ScheduleGroup({
  ...
  // 各surveyの回答数とNPS (Server から集計済みデータを受け取る)
  const enriched = items
    .map((sv) => ({
      sv,
      npsScore: npsBySurvey[sv.id],
      respCount: responseCountBySurvey[sv.id] ?? 0
    }))
  ...
  return (
    <div className="liquid-surface overflow-hidden">
      ...
      <ul className="divide-y divide-ink-50">
        {enriched.map(({ sv, npsScore, respCount }) => {
