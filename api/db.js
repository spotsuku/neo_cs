...
module.exports = async function handler(req, res) {
  ...
  if (body.action === 'all') {
    try {
      const rows = await sbReq(host, supabaseKey,
        '/rest/v1/ps_data?select=key,value&limit=1000', 'GET');
  ...
  if (body.action === 'del') {
    ...
    try {
      await sbReq(host, supabaseKey,
        `/rest/v1/ps_data?key=eq.${encodeURIComponent(key)}`, 'DELETE');
...
function sbReq(host, apiKey, path, method, body, extra = {}) {
  return new Promise((resolve, reject) => {
    ...
    const req = https.request(options, r => {
      ...
      r.on('data', c => { d += c; });
      r.on('end', () => {
        if (r.statusCode >= 400) reject(new Error(`Supabase ${r.statusCode}: ${d.slice(0,200)}`));
